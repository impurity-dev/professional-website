...
export class MenuGui {
    ...
    private createGui = (props: { mask: number }) => {
        const { mask } = props;
    ...
    private createTitle = (props: { scene: BABYLON.Scene }) => {
        const { scene } = props;
    ...
    private createFAQ = (props: { event: events.Events }): GUI.Button => {
        const { event } = props;
    ...
    private createCredits = (props: { event: events.Events }): GUI.Button => {
        const { event } = props;
        ...
        button.onPointerDownObservable.add(() => {
            event.onClick.notifyObservers();
            event.onCredits.notifyObservers({ toggle: true });
        });
        this.gui.addControl(button);
        return button;
    };

    private createStart = (props: { event: events.Events }): GUI.Button => {
        const { event } = props;
    ...
    private createOptions = (props: { event: events.Events }): GUI.Button => {
        const { event } = props;
    ...
    private createOptionMenu = (props: { scene: BABYLON.Scene; event: events.Events }) => {
